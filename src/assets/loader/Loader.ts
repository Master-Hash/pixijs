...
export class Loader
{
     ...
     */
    #getLoadPromiseAndParser(url: string, data?: ResolvedAsset): PromiseAndParser
    {
        ...
        result.promise = (async () =>
        {
            ...
            if (data.parser || data.loadParser)
            {
                ...
                parser = this.#parserHash[data.parser || data.loadParser];
    ...
    ): Promise<T | Record<string, T>>
    {
        ...
        const promises: Promise<void>[] = assetsToLoad.map(async (asset: ResolvedAsset) =>
        {
            ...
            await this.#loadAssetWithRetry(url, asset, { onProgress, onError, strategy, retryCount, retryDelay }, assets);
    ...
    )
    {
        ...
        while (true)
        {
            try
            {
                ...
                {
                    this.promiseCache[url] = this.#getLoadPromiseAndParser(url, asset);
                }
