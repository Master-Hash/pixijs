...
export class AssetsClass
{
    ...
    constructor()
    {
        ...
        this.#backgroundLoader = new BackgroundLoader(this.loader);
    ...
    ): Promise<T | Record<string, T>>
    {
        ...
        const out: Record<string, T> = await this.#mapLoadToResolve<T>(resolveResults, onProgress);
    ...
    public async loadBundle(bundleIds: ArrayOr<string>, onProgress?: ProgressCallback): Promise<any>
    {
        ...
        const promises = keys.map((bundleId, i) =>
        {
            ...
            return this.#mapLoadToResolve(resolveResult, (e) =>
            {
    ...
    public async backgroundLoad(urls: ArrayOr<string>): Promise<void>
    {
        ...
        this.#backgroundLoader.add(Object.values(resolveResults));
    }
    ...
    public async backgroundLoadBundle(bundleIds: ArrayOr<string>): Promise<void>
    {
        ...
        {
            this.#backgroundLoader.add(Object.values(resolveResult));
        });
    }
    ...
    ): Promise<void>
    {
        ...
        await this.#unloadFromResolved(resolveResults);
    }
    ...
    public async unloadBundle(bundleIds: ArrayOr<string>): Promise<void>
    {
        ...
        const promises = Object.keys(resolveResults).map((bundleId) =>
            this.#unloadFromResolved(resolveResults[bundleId]));

        await Promise.all(promises);
    }

    async #unloadFromResolved(resolveResult: ResolvedAsset | Record<string, ResolvedAsset>)
    {
