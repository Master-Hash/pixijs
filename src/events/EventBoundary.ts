...
export class EventBoundary
{
    ...
    readonly #allInteractiveElements: Container[] = [];
    /** Every element that passed the hit test. Only used in `pointermove` */
    #hitElements: Container[] = [];
    /** Whether or not to collect all the interactive elements from the scene. Enabled in `pointermove` */
    #isPointerMoveEvent = false;
    ...
    ): Container
    {
        ...
        const useMove = this.#isPointerMoveEvent && this.enableGlobalMoveEvents;
     ...
     */
    public all(e: FederatedEvent, type?: string | string[], targets = this.#allInteractiveElements): void
    {
    ...
    ): Container[]
    {
        ...
        if (this.#interactivePrune(currentTarget)) return null;
        ...
        if (currentTarget.interactiveChildren && currentTarget.children)
        {
            ...
            for (let i = children.length - 1; i >= 0; i--)
            {
                ...
                const nestedHit = this.hitTestMoveRecursive(
                    child,
                    this.#isInteractive(eventMode) ? eventMode : child.eventMode,
                    location,
                    testFn,
                    pruneFn,
                ...
                if (nestedHit)
                {
                    ...
                    {
                        if (isInteractive) this.#allInteractiveElements.push(currentTarget);
                        nestedHit.push(currentTarget);
                    }

                    // store all hit elements to be returned once we have traversed the whole tree
                    if (this.#hitElements.length === 0) this.#hitElements = nestedHit;
        ...
        const isInteractiveMode = this.#isInteractive(eventMode);
        const isInteractiveTarget = currentTarget.isInteractive();

        if (isInteractiveTarget && isInteractiveTarget) this.#allInteractiveElements.push(currentTarget);

        // we don't carry on hit testing something once we have found a hit,
        // now only care about gathering the interactive elements
        if (ignore || this.#hitElements.length > 0) return null;

        if (shouldReturn) return this.#hitElements as Container[];
    ...
    ): Container[]
    {
        ...
        if (this.#interactivePrune(currentTarget) || pruneFn(currentTarget, location))
        {
            return null;
        }
        if (currentTarget.eventMode === 'dynamic' || eventMode === 'dynamic')
    ...
    #isInteractive(int: EventMode): int is 'static' | 'dynamic'
    {
        return int === 'static' || int === 'dynamic';
    }

    #interactivePrune(container: Container): boolean
    {
    ...
    protected mapPointerMove(from: FederatedEvent): void
    {
        ...
        this.#allInteractiveElements.length = 0;
        this.#hitElements.length = 0;
        this.#isPointerMoveEvent = true;
        const e = this.createPointerEvent(from);

        this.#isPointerMoveEvent = false;
     ...
     */
    #notifyListeners(e: FederatedEvent, type: string): void
    {
