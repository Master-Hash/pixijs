...
export class FilterSystem implements System
{
    ...
    readonly #filterGlobalUniforms = new UniformGroup({
    ...
    readonly #globalFilterBindGroup: BindGroup = new BindGroup({});
    ...
    {
        return this.#activeFilterData?.backTexture;
    }
    ...
    public push(instruction: FilterInstruction)
    {
        ...
        this.#calculateFilterArea(instruction, bounds);

        this.#calculateFilterBounds(filterData, renderer.renderTarget.rootViewPort, rootAntialias, rootResolution, 1);
        ...
        this.#setupFilterTextures(filterData, bounds, renderer, previousFilterData);
    }
    ...
    public generateFilteredTexture({ texture, filters }: {texture: Texture, filters: Filter[]}): Texture
    {
        ...
        this.#calculateFilterBounds(filterData, bounds.rectangle, rootAntialias, rootResolution, 0);
        ...
        this.#applyFiltersToTexture(filterData, true);
    ...
    public applyFilter(filter: Filter, input: Texture, output: RenderSurface, clear: boolean)
    {
        ...
        if (isFinalTarget)
        {
            const offset = this.#findPreviousFilterOffset();

            offsetX = offset.x;
            offsetY = offset.y;
        }

        this.#updateFilterUniforms(input, output, filterData, offsetX, offsetY, resolution, isFinalTarget, clear);
        ...
        const filterToApply = filter.enabled
            ? filter
            : this.#getPassthroughFilter();

        this.#setupBindGroupsAndRender(filterToApply, input, renderer);
    }
    ...
    public destroy(): void
    {
        this.#passthroughFilter?.destroy(true);
        (this.#passthroughFilter as null) = null;
    }

    #getPassthroughFilter(): Filter
    {
        this.#passthroughFilter ??= new PassthroughFilter();

        return this.#passthroughFilter;
    }
     ...
     */
    #setupBindGroupsAndRender(filter: Filter, input: Texture, renderer: Renderer): void
    {
        // TODO - should prolly use a adaptor...
        if ((renderer as WebGPURenderer).renderPipes.uniformBatch)
        {
            const batchUniforms = (renderer as WebGPURenderer).renderPipes.uniformBatch
                .getUboResource(this.#filterGlobalUniforms);

            this.#globalFilterBindGroup.setResource(batchUniforms, 0);
        }
        else
        {
            this.#globalFilterBindGroup.setResource(this.#filterGlobalUniforms, 0);
        }
        ...
        this.#globalFilterBindGroup.setResource(input.source.style, 2);
     ...
     */
    #findFilterResolution(rootResolution: number): number
    {
        let currentIndex = this.#filterStackIndex - 1;

        while (currentIndex > 0 && this.#filterStack[currentIndex].skip)
        {
            --currentIndex;
        }

        return currentIndex > 0 && this.#filterStack[currentIndex].inputTexture
            ? this.#filterStack[currentIndex].inputTexture.source._resolution
            : rootResolution;
    }
     ...
     */
    #findPreviousFilterOffset(): { x: number, y: number }
    {
        let offsetX = 0;
        let offsetY = 0;
        let lastIndex = this.#filterStackIndex;

        while (lastIndex > 0)
        {
            lastIndex--;
            const prevFilterData = this.#filterStack[lastIndex];
     ...
     */
    #calculateFilterArea(instruction: FilterInstruction, bounds: Bounds): void
    {
        // this path is used by the blend modes mostly!
        // they collect all renderables and push them into a list.
        // this list is then used to calculate the bounds of the filter area
    ...
    #applyFiltersToTexture(filterData: FilterData, clear: boolean)
    {
        ...
        this.#globalFilterBindGroup.setResource(inputTexture.source.style, 2);
        this.#globalFilterBindGroup.setResource(filterData.backTexture.source, 3);
    ...
    #getPreviousFilterData(): FilterData | null
    {
        ...
        {
            index--;
            previousFilterData = this.#filterStack[index];
    ...
    {
        let filterData = this.#filterStack[this.#filterStackIndex];

        if (!filterData)
        {
            filterData = this.#filterStack[this.#filterStackIndex] = new FilterData();
        }
